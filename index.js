...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i <= 8; i++) {
    pass = pass.concat(characters[Math.floor(Math.random() * 62)]);
  }
...
function roundObjectValues(obj) {
  ...
  let arr = Object.entries(obj).map(pair => {
    let key = pair[0];
    let value = pair[1];
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  if (typeof products !== 'object' && typeof maxPrice !== 'number') {
    return null;
  }
